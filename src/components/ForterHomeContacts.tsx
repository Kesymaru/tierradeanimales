...
import { FosterHomeContact } from "@models/fosterHome";
import { INIT_HOME_CONTACT } from "@constants/fosterHome";

export interface HomeContactsProps {
  contacts: Array<FosterHomeContact>;
  disabled?: boolean;
  onChange?: (contacts: Array<FosterHomeContact>) => void;
}

const HomeContacts: FunctionComponent<HomeContactsProps> = (props) => {
  const [contacts, setContacts] = useState<FosterHomeContact[]>(props.contacts);
  useEffect(() => {
    setContacts(props.contacts);
  }, [props]);

  function handleAdd(event: MouseEvent<HTMLButtonElement>) {
    const _contacts = [...contacts, { ...INIT_HOME_CONTACT }];
  ...
  function handleChange(index: number, field: keyof FosterHomeContact) {
