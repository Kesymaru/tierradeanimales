...
export interface IAppTableProps
  extends Omit<IAppTableHeaderProps, "total" | "selected" | "cells">,
    Omit<IAppTableFooterProps, "pagination"> {
  data: any[];
...
function AppTable(props: IAppTableProps) {
  ...
  function handleSelect(row: any) {
    return (event: MouseEvent<unknown>) => {
      row._selected =
        typeof row._selected === "undefined" ? true : !row._selected;
              ...
              {data.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={cells.length + 1}>
                    {loading ? <LinearProgress color="primary" /> : "No Data"}
                  </TableCell>
                </TableRow>
              ) : null}
